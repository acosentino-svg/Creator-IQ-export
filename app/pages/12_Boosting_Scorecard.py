"""Boosting Program Scorecard — CreatorIQ API + monthly CSV paste-in."""
...
from creatoriq_dashboard.boosting_data_access import (  # noqa: E402
    load_boosting_content,
    rebuild_boosting_from_cached_posts,
    save_boosting_content,
    sync_and_store_boosting_content,
)
from creatoriq_dashboard.boosting_demo_data import generate_demo_boosting_content  # noqa: E402
...
def _get_content() -> pd.DataFrame:
    if SESSION_KEY not in st.session_state:
        content, _ = load_boosting_content(get_config())
...
sync_info, _ = load_boosting_content(config)
...
st.caption(
    "Publisher ID is the stable creator key. In **live mode**, content loads from CreatorIQ campaign "
    "activity (Boosting campaigns). Paste monthly CSV exports to supplement paid spend / revenue."
)

if config.is_demo:
    st.info("**Demo mode** — showing synthetic Boosting data. Set `CREATORIQ_DASHBOARD_MODE=live` and your API key to pull real data.")
else:
    posts_synced = sync_info.get("posts")
    boosting_synced = sync_info.get("boosting_content")
    st.success(
        f"**Live mode** — posts last synced: `{posts_synced or 'never'}` · "
        f"boosting content: `{boosting_synced or 'never'}`"
    )
...
with st.expander("Data sources & sync", expanded=not config.is_demo):
    ...
    st.markdown(
        "Configure which campaigns count as Boosting in `config/settings.yaml` → `boosting.campaign_name_contains` "
        "or `boosting.campaign_ids`. Map CreatorIQ custom fields in `config/field_mappings.yaml` → `posts`."
    )
    ...
    if not kpis:
        ...
        with chart_left:
            ...
            if not trends.empty:
                ...
                fig.add_trace(go.Bar(name="Reactivated", x=trends["month_label"], y=trends.get("reactivated_creators", 0)))
